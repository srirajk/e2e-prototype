...
INSERT INTO business_product (id, business_id, product_id, split_size, field_length)
SELECT 100, 'business123', 'product123', 10, 13
WHERE NOT EXISTS (
    SELECT 1 FROM business_product WHERE business_id = 'business123' AND product_id = 'product123'
);

INSERT INTO business_product (id, business_id, product_id, split_size, field_length)
SELECT 101, 'business123', 'productabc', 20, 13
WHERE NOT EXISTS (
    SELECT 1 FROM business_product WHERE business_id = 'business123' AND product_id = 'productabc'
);

INSERT INTO business_product (id, business_id, product_id, split_size, field_length)
SELECT 102, 'business999', 'product3', 30, 13
WHERE NOT EXISTS (
